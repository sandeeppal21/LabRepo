...
import {
    ...
    RiShieldCheckLine, RiUserLine, RiLockLine,
} from "react-icons/ri";
...
// Status filter sets differ by role — vendor filters by their own
// pricing state, admin filters by who added the test.
const VENDOR_STATUS_FILTERS = [["ALL", "All"], ["PRICED", "Priced"], ["UNPRICED", "Unpriced"], ["ACTIVE", "Active"]];
const ADMIN_STATUS_FILTERS = [["ALL", "All"], ["ADMIN_ADDED", "Admin Added"], ["VENDOR_ADDED", "Vendor Added"]];
...
function useTests(role) {
    ...
    const handleSaved = useCallback((saved, isEdit) => {
        setTests(ts => isEdit
            ...
            : [
                role === "vendor"
                    ? { ...saved, vendorPrice: null, priceSet: false, vendorIsActive: false }
                    : { ...saved },
                ...ts,
            ]);
...
/* ══════════════════════════════════════════════════════════════════
   HOOK — search / filter / sort / stats  (role-aware)
   ══════════════════════════════════════════════════════════════════ */

function useTestFilters(tests, role) {
    ...
    const visible = useMemo(() => {
        ...
        if (role === "vendor") {
            ...
            if (statusFilter === "ADMIN_ADDED") out = out.filter(t => t.createdByRole === "admin");
            if (statusFilter === "VENDOR_ADDED") out = out.filter(t => t.createdByRole === "vendor");
        }
    ...
    }, [tests, deferredSearch, deptFilter, statusFilter, sortKey, sortDir, role]);

    // Single pass instead of several separate .filter().length calls.
    const stats = useMemo(() => {
        if (role === "vendor") {
        ...
        let adminAdded = 0, vendorAdded = 0, params = 0;
        for (const t of tests) {
            if (t.createdByRole === "admin") adminAdded++;
            else if (t.createdByRole === "vendor") vendorAdded++;
            if (t.parameters) for (const p of t.parameters) if (p.fieldType !== "heading") params++;
        }
        return { total: tests.length, adminAdded, vendorAdded, params };
    }, [tests, role]);
...
/* ══════════════════════════════════════════════════════════════════
   PRICE EDITOR + TOGGLE (memoized) — vendor-only
   ══════════════════════════════════════════════════════════════════ */
...
/* Admin-only: who added the test, in place of the price/toggle columns */
const AddedByBadge = memo(function AddedByBadge({ test, t }) {
    const isAdminAdded = test.createdByRole === "admin";
    return (
        <span style={{
            display: "inline-flex", alignItems: "center", gap: 5, width: "fit-content",
            fontSize: "0.72rem", fontWeight: 600, padding: "4px 9px", borderRadius: 7,
            color: isAdminAdded ? t.accent : t.muted,
            background: isAdminAdded ? t.accentBg : "rgba(148,163,184,0.12)",
            border: `1px solid ${isAdminAdded ? t.accentRing : t.border}`,
        }}>
            {isAdminAdded ? <RiShieldCheckLine size={12} /> : <RiUserLine size={12} />}
            {isAdminAdded ? "Admin" : "Vendor"}
        </span>
    );
});
...
const TestRow = memo(function TestRow({ test, t, style, isLast, role, currentUserId, onEdit, onPriceSaved, onToggled, onDeleted, onToast }) {
    ...
    const isCreator = !!currentUserId && !!rawOwnerId && String(rawOwnerId) === String(currentUserId);


    const canManage = isAdmin || isCreator;

    const gridTemplateColumns = isAdmin
        ? "2.3fr 1fr 1fr 0.65fr 0.55fr 0.55fr 0.9fr 1.1fr"
        : "2.1fr 1fr 1fr 0.65fr 0.55fr 0.55fr 0.85fr 0.55fr 1.1fr";
    ...
    return (
        <div className="cat-row" style={{
            ...
            {isAdmin ? (
                <AddedByBadge test={test} t={t} />
            ) : (
                <PriceEditor test={test} t={t} onSaved={onPriceSaved} onToast={onToast} />
            )}
            {!isAdmin && <Toggle test={test} t={t} onToggled={onToggled} onToast={onToast} />}
            ...
            <div style={{ display: "flex", gap: 6 }}>
                {canManage ? (
                    <>
                        ...
                        <RowDeleteButton testId={test._id} t={t} onDeleted={onDeleted} onToast={onToast} />
                    </>
                ) : (
                    <span title="Only the creator or admin can edit/delete this test"
                        style={{ width: 30, height: 30, borderRadius: 7, border: `1px solid ${t.border}`, display: "flex", alignItems: "center", justifyContent: "center", color: t.faint }}>
                        <RiLockLine size={13} />
                    </span>
                )}
            </div>
...
function VirtualTestList({ visible, t, role, currentUserId, onEdit, onPriceSaved, onToggled, onDeleted, onToast }) {
                ...
                {rowVirtualizer.getVirtualItems().map(vItem => {
                    ...
                    return (
                        <TestRow
                            ...
                            role={role} currentUserId={currentUserId}
...
function VendorTestInner({ t, role, currentUserId }) {
    const isAdmin = role === "admin";
    const showToast = useToast();
    const { tests, loading, error, load, handleSaved, handlePriceSaved, handleToggled, handleDeleted } = useTests(role);
    const {
        search, setSearch, deptFilter, setDeptFilter, statusFilter, setStatusFilter,
        sortKey, sortDir, handleSort, departments, visible, stats, clearFilters, hasActiveFilters,
    } = useTestFilters(tests, role);
    ...
    const STATUS_FILTERS = isAdmin ? ADMIN_STATUS_FILTERS : VENDOR_STATUS_FILTERS;
    ...
    const headerGridTemplateColumns = isAdmin
        ? "2.3fr 1fr 1fr 0.65fr 0.55fr 0.55fr 0.9fr 1.1fr"
        : "2.1fr 1fr 1fr 0.65fr 0.55fr 0.55fr 0.85fr 0.55fr 1.1fr";
    ...
    const statCards = isAdmin
        ? [
            ...
            { label: "Admin Added", value: stats.adminAdded, color: "#0ea5e9" },
            { label: "Vendor Added", value: stats.vendorAdded, color: "#16a34a" },
      ...
      `}</style>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 22, flexWrap: "wrap", gap: 12 }}>
                <div>
                    ...
                    <p style={{ fontSize: "0.84rem", color: t.muted }}>
                        {isAdmin
                            ? "Global library shared with every vendor. You can edit or delete any test; vendors can only manage tests they added."
                            : "Global library — add tests with parameters, ranges and interpretation. Set your own prices."}
                    </p>
                </div>
            ...
            {loading ? (
                ...
                <div style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: 16, overflow: "hidden" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: headerGridTemplateColumns, padding: "9px 20px", background: t.accentBg, borderBottom: `1px solid ${t.border}`, fontSize: "0.66rem", fontWeight: 600, color: t.muted, letterSpacing: "0.07em", textTransform: "uppercase" }}>
                        ...
                        {isAdmin ? <span>Added By</span> : <ColH label="Your Price" k="vendorPrice" />}
                        {!isAdmin && <span>Active</span>}
                        <span>Actions</span>
                    </div>

                    <VirtualTestList
                        visible={visible} t={t} role={role} currentUserId={currentUserId}
            ...
            {!loading && !error && tests.length > 0 && (
                <p style={{ fontSize: "0.72rem", color: t.faint, marginTop: 14, textAlign: "center" }}>
                    {isAdmin
                        ? "🔒 Lock icon = you didn't add this test and can't manage it · Click edit icon to update parameters and interpretation"
...
export default function VendorTest({ t, role, currentUserId, currentVendorId }) {
    // Backward compatible: existing <VendorTest t={t} currentVendorId={id} />
    // call sites keep working unchanged (role defaults to "vendor").
    const resolvedRole = role || "vendor";
    const resolvedUserId = currentUserId ?? currentVendorId;

    return (
        <ToastProvider>
            <VendorTestInner t={t} role={resolvedRole} currentUserId={resolvedUserId} />
        </ToastProvider>
    );
}