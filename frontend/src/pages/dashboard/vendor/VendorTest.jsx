// /**
//  * VendorCatalogue.jsx
//  * Full test catalogue with:
//  *  - Rich test builder (parameters, units, ranges, field types)
//  *  - Interpretation editor
//  *  - Edit full test
//  *  - Inline price setting
...
//     RiPencilLine, RiSaveLine, RiFilterLine, RiFlaskLine,
//     RiArrowUpLine, RiArrowDownLine, RiRefreshLine,
//     RiDeleteBinLine, RiDraggable, RiEyeLine, RiArrowRightLine,
//     RiFileTextLine, RiSettings3Line,
// } from "react-icons/ri";
// import { fetchTests, addTest, updateTest, setPrice, toggleTest } from "../../../services/testServices";

// const DEPARTMENTS = [
//     "HAEMATOLOGY", "BIOCHEMISTRY", "IMMUNOASSAY", "MICROBIOLOGY",
//     "PATHOLOGY", "ENDOCRINOLOGY", "COAGULATION", "SEROLOGY", "URINE ANALYSIS", "OTHER",
// ];
// const FIELD_TYPES = ["numeric", "text", "option"];
// const GENDERS = ["both", "male", "female"];

// // ── Helpers ────────────────────────────────────────────────────────
// const uid = () => Math.random().toString(36).slice(2, 9);
// const emptyParam = () => ({
//     _uid: uid(), name: "", fieldType: "numeric",
//     unit: "", rangeMin: "", rangeMax: "", rangeText: "",
//     isSubField: false, showInReport: true,
// });
...
//                     color: t.type === "success" ? "#16a34a" : "#ef4444",
//                     fontSize: "0.84rem", fontWeight: 500,
//                     boxShadow: "0 4px 20px rgba(0,0,0,0.2)",
//                     animation: "slideIn .2s ease",
//                 }}>
//                     {t.type === "success" ? <RiCheckboxCircleLine size={16} /> : <RiAlertLine size={16} />}
//                     {t.message}
//                 </div>
//             ))}
//         </div>
...
// // ── Parameter Row ──────────────────────────────────────────────────
// function ParamRow({ param, idx, t, onChange, onDelete, onToggleSub }) {
//     const inp = (key, props = {}) => (
//         <input
//             value={param[key]}
//             onChange={e => onChange(idx, key, e.target.value)}
//             style={{ background: t.inputBg, border: `1px solid ${t.accentRing}`, borderRadius: 7, padding: "6px 9px", color: t.text, fontSize: "0.8rem", fontFamily: "'DM Sans',sans-serif", outline: "none", width: "100%" }}
//             {...props}
//         />
//     );
...
//         <div style={{
//             display: "grid",
//             gridTemplateColumns: "20px 1.8fr 0.9fr 0.7fr 0.6fr 0.6fr 1fr 60px",
//             gap: 6, alignItems: "center", padding: "8px 12px",
//             background: param.isSubField ? "rgba(56,189,248,0.03)" : "transparent",
//             borderRadius: 8,
//             borderLeft: param.isSubField ? `3px solid ${t.accentRing}` : "3px solid transparent",
//             marginLeft: param.isSubField ? 16 : 0,
//         }}>
//             {/* Drag handle */}
//             <RiDraggable size={14} style={{ color: t.faint, cursor: "grab", flexShrink: 0 }} />

//             {/* Name */}
//             {inp("name", { placeholder: "Parameter name" })}

//             {/* Field type */}
//             <select value={param.fieldType} onChange={e => onChange(idx, "fieldType", e.target.value)}
//                 style={{ background: t.inputBg, border: `1px solid ${t.accentRing}`, borderRadius: 7, padding: "6px 9px", color: t.text, fontSize: "0.8rem", fontFamily: "'DM Sans',sans-serif", outline: "none" }}>
//                 {FIELD_TYPES.map(f => <option key={f} value={f}>{f}</option>)}
//             </select>

//             {/* Unit */}
//             {inp("unit", { placeholder: "Unit" })}

//             {/* Range min */}
//             {param.fieldType === "numeric"
//                 ? inp("rangeMin", { type: "number", placeholder: "Min" })
//                 : <span style={{ fontSize: "0.72rem", color: t.faint, textAlign: "center" }}>—</span>
//             }

//             {/* Range max */}
//             {param.fieldType === "numeric"
//                 ? inp("rangeMax", { type: "number", placeholder: "Max" })
//                 : <span />
//             }

//             {/* Range text (for non-numeric or display override) */}
//             {inp("rangeText", { placeholder: param.fieldType === "option" ? "e.g. Negative/Positive" : "Range display" })}

//             {/* Actions */}
//             <div style={{ display: "flex", gap: 4, justifyContent: "flex-end" }}>
//                 <button type="button" onClick={() => onToggleSub(idx)} title={param.isSubField ? "Make main field" : "Make sub-field"}
//                     style={{ width: 26, height: 26, borderRadius: 6, border: `1px solid ${param.isSubField ? t.accent : t.border}`, background: param.isSubField ? t.accentBg : "none", color: param.isSubField ? t.accent : t.muted, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.6rem", fontWeight: 700 }}>
//                     ↳
//                 </button>
...
// function TestFormModal({ t, isDark, editTest, onClose, onSuccess }) {
//     const isEdit = !!editTest;
//     const [tab, setTab] = useState("info");   // info | parameters | interpretation
...
//         editTest?.parameters?.length
//             ? editTest.parameters.map(p => ({ ...p, _uid: uid() }))
//             : [emptyParam()]
//     );
...
//     const changeParam = (i, k, v) => setParams(p => p.map((r, j) => j === i ? { ...r, [k]: v } : r));
//     const toggleSub = (i) => setParams(p => p.map((r, j) => j === i ? { ...r, isSubField: !r.isSubField } : r));
...
//             setSaving(true);
//             let res;
//             if (isEdit) res = await updateTest(editTest._id, payload);
//             else res = await addTest(payload);
//             onSuccess(res.data.test, isEdit);
//             onClose();
//         } catch (err) {
//             const data = err.response?.data;
//             if (data?.errors) setErrors(data.errors);
//             else setErrors({ name: data?.message || "Failed to save test." });
...
//             background: tab === key ? t.accentBg : "none",
//             color: tab === key ? t.accent : t.navText,
...
//                 borderRadius: 20, boxShadow: "0 28px 70px rgba(0,0,0,0.45)",
...
//                         <div>
//                             <div className="playfair" style={{ fontSize: "1.15rem", fontWeight: 700, color: t.heading }}>
//                                 {isEdit ? `Edit: ${editTest.name}` : "Add New Test"}
//                             </div>
//                             <div style={{ fontSize: "0.76rem", color: t.muted, marginTop: 2 }}>
//                                 {isEdit ? "Edit test details, parameters and interpretation" : "Add to global library — all vendors can see and price this test"}
//                             </div>
//                         </div>
...
//                         {tabBtn("info", "Test Info", RiSettings3Line)}
//                         {tabBtn("parameters", "Parameters", RiFlaskLine)}
//                         {tabBtn("interpretation", "Interpretation", RiFileTextLine)}
//                     </div>
//                 </div>
...
//                                     <input value={info.name} onChange={e => setInfoField("name", e.target.value)} placeholder="e.g. Complete Blood Count" style={inpStyle("name")} />
...
//                                     <input value={info.code} onChange={e => setInfoField("code", e.target.value.toUpperCase())} placeholder="e.g. CBC-001" style={inpStyle("code")} />
...
//                                     <select value={info.department} onChange={e => setInfoField("department", e.target.value)} style={{ ...inpStyle("department"), cursor: "pointer" }}>
...
//                                     <input value={info.sampleType} onChange={e => setInfoField("sampleType", e.target.value)} placeholder="Blood, Serum, Urine…" style={inpStyle("sampleType")} />
//                                 </div>
//                                 {/* TAT */}
//                                 <div>
...
//                                     <input value={info.tat} onChange={e => setInfoField("tat", e.target.value)} placeholder="4 hrs, Same day…" style={inpStyle("tat")} />
//                                 </div>
//                             </div>
...
//                                     {GENDERS.map(g => (
//                                         <label key={g} style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer", fontSize: "0.84rem", color: t.text }}>
//                                             <input type="radio" name="gender" value={g} checked={info.gender === g} onChange={() => setInfoField("gender", g)}
...
//                                 <textarea value={info.description} onChange={e => setInfoField("description", e.target.value)} rows={2} placeholder="Brief description (optional)" style={{ ...inpStyle("description"), resize: "vertical" }} />
//                             </div>
//                         </>
//                     )}
...
//                                 {params.map((p, i) => (
//                                     <ParamRow key={p._uid} param={p} idx={i} t={t}
//                                         onChange={changeParam} onDelete={delParam} onToggleSub={toggleSub} />
//                                 ))}
//                             </div>

//                             {/* Add parameter button */}
//                             <button type="button" onClick={addParam} style={{
//                                 display: "flex", alignItems: "center", gap: 7,
//                                 padding: "9px 16px", borderRadius: 9,
//                                 border: `1.5px dashed ${t.accentRing}`,
...
//                             {/* Hint */}
//                             <p style={{ fontSize: "0.72rem", color: t.faint }}>
//                                 ↳ = Sub-field (indented in report) · Numeric: enter min/max · Text/Option: use Range/Options field
//                             </p>
//                         </>
//                     )}
...
//                                     <label style={{ fontSize: "0.68rem", fontWeight: 600, color: t.muted, letterSpacing: "0.08em", textTransform: "uppercase" }}>
//                                         Interpretation Template
//                                     </label>
//                                     <span style={{ fontSize: "0.72rem", color: t.faint }}>Shown on the report below test results</span>
//                                 </div>
//                                 <textarea
//                                     value={interpretation}
//                                     onChange={e => setInterpretation(e.target.value)}
//                                     rows={12}
//                                     placeholder={`Enter interpretation text for this test.\n\nExample:\nNormal haemoglobin levels indicate adequate oxygen-carrying capacity. Low values may suggest anaemia. Elevated values may indicate polycythaemia.\n\nConsult your physician for detailed evaluation.`}
//                                     style={{
//                                         width: "100%", background: t.inputBg,
...
//                             {/* Preview */}
//                             {interpretation.trim() && (
//                                 <div style={{ background: t.accentBg, border: `1px solid ${t.accentRing}`, borderRadius: 10, padding: 16 }}>
//                                     <div style={{ fontSize: "0.68rem", fontWeight: 600, color: t.muted, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 8 }}>Preview</div>
//                                     <div style={{ fontSize: "0.84rem", color: t.text, lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{interpretation}</div>
//                                 </div>
//                             )}
//                         </>
//                     )}
...
//                         {["info", "parameters", "interpretation"].map((k, i) => (
//                             <div key={k} onClick={() => setTab(k)} style={{
//                                 width: 8, height: 8, borderRadius: "50%", cursor: "pointer",
//                                 background: tab === k ? t.accent : t.border,
//                                 transition: "background .2s",
//                             }} />
//                         ))}
//                     </div>
//                     <div style={{ display: "flex", gap: 10 }}>
//                         <button type="button" onClick={onClose} style={{ padding: "9px 20px", borderRadius: 9, border: `1px solid ${t.border}`, background: "none", color: t.muted, fontSize: "0.86rem", cursor: "pointer", fontFamily: "'DM Sans',sans-serif" }}>
//                             Cancel
//                         </button>
//                         {tab !== "interpretation"
//                             ? <button type="button" onClick={() => setTab(tab === "info" ? "parameters" : "interpretation")}
//                                 style={{ padding: "9px 20px", borderRadius: 9, border: "none", background: t.accentBg, color: t.accent, fontSize: "0.86rem", fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", display: "flex", alignItems: "center", gap: 6 }}>
//                                 Next <RiArrowRightLine size={14} />
//                             </button>
//                             : <button onClick={handleSubmit} disabled={saving}
//                                 style={{ padding: "9px 22px", borderRadius: 9, border: "none", background: t.accent, color: "#fff", fontSize: "0.86rem", fontWeight: 600, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.7 : 1, display: "flex", alignItems: "center", gap: 7, fontFamily: "'DM Sans',sans-serif" }}>
...
// // ── Inline Price Editor ────────────────────────────────────────────
// function PriceEditor({ test, t, onPriceSaved, onToast }) {
...
//             onPriceSaved(test._id, Number(value));
...
//             <span style={{ fontSize: "0.82rem", color: t.muted }}>₹</span>
//             <input ref={ref} type="number" min="0" value={value}
//                 onChange={e => setValue(e.target.value)}
//                 onKeyDown={e => { if (e.key === "Enter") save(); if (e.key === "Escape") setEditing(false); }}
//                 style={{ width: 76, background: t.inputBg, border: `1.5px solid ${t.accent}`, borderRadius: 7, padding: "5px 8px", color: t.text, fontSize: "0.84rem", fontFamily: "'DM Sans',sans-serif", outline: "none" }}
//             />
//             <button onClick={save} disabled={saving} style={{ background: t.accent, border: "none", borderRadius: 7, width: 28, height: 28, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#fff" }}>
//                 {saving ? <RiLoader4Line size={13} style={{ animation: "spin .7s linear infinite" }} /> : <RiSaveLine size={13} />}
//             </button>
//             <button onClick={() => setEditing(false)} style={{ background: "none", border: `1px solid ${t.border}`, borderRadius: 7, width: 28, height: 28, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: t.muted }}>
//                 <RiCloseLine size={13} />
//             </button>
//         </div>
//     );
...
//             <span style={{ fontSize: "0.88rem", fontWeight: 600, color: test.priceSet ? t.heading : t.faint }}>
//                 {test.priceSet ? `₹${test.vendorPrice}` : "—"}
//             </span>
...
// // ── Toggle ─────────────────────────────────────────────────────────
// function Toggle({ test, t, onToggled, onToast }) {
//     const [loading, setLoading] = useState(false);
//     const isOn = test.vendorIsActive && test.priceSet;

//     const handle = async () => {
//         if (!test.priceSet) { onToast("Set a price first.", "error"); return; }
//         try {
//             setLoading(true);
//             await toggleTest(test._id);
//             onToggled(test._id);
//         } catch (err) {
//             onToast(err.userMessage || "Failed.", "error");
...
// // ══════════════════════════════════════════════════════════════════
// export default function VendorTest({ t, isDark }) {
...
//     const [modal, setModal] = useState(null);  // null | "add" | testObj (edit)
...
//         try { setLoading(true); setError(""); const r = await fetchTests(); setTests(r.data.tests); }
//         catch (err) { setError(err.userMessage || err.response?.data?.message || "Failed to load."); }
//         finally { setLoading(false); }
//     }, []);
...
//         let out = [...tests];
//         if (search.trim()) { const q = search.toLowerCase(); out = out.filter(t => t.name.toLowerCase().includes(q) || t.code.toLowerCase().includes(q) || t.department?.toLowerCase().includes(q)); }
...
//         out.sort((a, b) => {
//             let av = sortKey === "vendorPrice" ? (a.vendorPrice ?? -1) : (a[sortKey] ?? "");
//             let bv = sortKey === "vendorPrice" ? (b.vendorPrice ?? -1) : (b[sortKey] ?? "");
//             const c = typeof av === "string" ? av.localeCompare(bv) : av - bv;
//             return sortDir === "asc" ? c : -c;
//         });
//         return out;
//     }, [tests, search, deptFilter, statusFilter, sortKey, sortDir]);

//     const stats = useMemo(() => ({
//         total: tests.length,
//         priced: tests.filter(t => t.priceSet).length,
//         active: tests.filter(t => t.vendorIsActive && t.priceSet).length,
//         params: tests.reduce((s, t) => s + (t.parameters?.length || 0), 0),
//     }), [tests]);

//     const handleSaved = useCallback((saved, isEdit) => {
//         if (isEdit) {
//             setTests(ts => ts.map(t => t._id === saved._id ? { ...saved, vendorPrice: t.vendorPrice, priceSet: t.priceSet, vendorIsActive: t.vendorIsActive } : t));
//             showToast("Test updated successfully.", "success");
//         } else {
//             setTests(ts => [{ ...saved, vendorPrice: null, priceSet: false, vendorIsActive: false }, ...ts]);
//             showToast("Test added to global library!", "success");
//         }
//     }, [showToast]);
...
//         : <RiArrowUpLine size={11} style={{ opacity: 0.2 }} />;
...
//             {modal === "add" && <TestFormModal t={t} isDark={isDark} editTest={null} onClose={() => setModal(null)} onSuccess={handleSaved} />}
//             {modal && modal !== "add" && <TestFormModal t={t} isDark={isDark} editTest={modal} onClose={() => setModal(null)} onSuccess={handleSaved} />}
...
//                     </button>
//                     <button onClick={() => setModal("add")} style={{ display: "flex", alignItems: "center", gap: 7, background: t.accent, border: "none", borderRadius: 9, padding: "9px 18px", color: "#fff", fontSize: "0.85rem", fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans',sans-serif" }}>
...
//                     { label: "Total Params", value: stats.params, color: t.muted },
//                 ].map(s => (
...
//                     style={{ background: t.inputBg, border: `1px solid ${t.accentRing}`, borderRadius: 10, padding: "8px 12px", color: t.text, fontSize: "0.83rem", fontFamily: "'DM Sans',sans-serif", outline: "none", cursor: "pointer" }}>
//                     {departments.map(d => <option key={d} value={d}>{d === "ALL" ? "All Departments" : d}</option>)}
//                 </select>
//                 <div style={{ display: "flex", gap: 4 }}>
//                     {[["ALL", "All"], ["PRICED", "Priced"], ["UNPRICED", "Unpriced"], ["ACTIVE", "Active"]].map(([k, l]) => (
//                         <button key={k} onClick={() => setStatusFilter(k)} style={{
//                             padding: "7px 13px", borderRadius: 8, fontSize: "0.78rem", fontWeight: 500,
//                             border: `1px solid ${statusFilter === k ? t.accent : t.border}`,
//                             background: statusFilter === k ? t.accentBg : "none",
//                             color: statusFilter === k ? t.accent : t.navText,
...
//                             Showing <strong style={{ color: t.text }}>{visible.length}</strong> of <strong style={{ color: t.text }}>{tests.length}</strong> tests
//                         </span>
//                         {(search || deptFilter !== "ALL" || statusFilter !== "ALL") && (
//                             <button onClick={() => { setSearch(""); setDeptFilter("ALL"); setStatusFilter("ALL"); }} style={{ fontSize: "0.74rem", color: t.accent, background: "none", border: "none", cursor: "pointer" }}>Clear filters</button>
//                         )}
//                     </div>

//                     {/* Column headers */}
//                     <div style={{ display: "grid", gridTemplateColumns: "2.2fr 1fr 1.1fr 0.7fr 0.7fr 0.7fr 0.9fr 0.6fr 0.6fr", padding: "9px 20px", background: t.accentBg, borderBottom: `1px solid ${t.border}`, fontSize: "0.67rem", fontWeight: 600, color: t.muted, letterSpacing: "0.07em", textTransform: "uppercase" }}>
...
//                     {/* Rows */}
//                     {visible.map((test, i) => (
//                         <div key={test._id} className="cat-row" style={{ display: "grid", gridTemplateColumns: "2.2fr 1fr 1.1fr 0.7fr 0.7fr 0.7fr 0.9fr 0.6fr 0.6fr", padding: "12px 20px", borderBottom: i < visible.length - 1 ? `1px solid ${t.border}` : "none", alignItems: "center", transition: "background .12s" }}>

//                             {/* Name + description */}
//                             <div>
//                                 <div style={{ fontWeight: 500, color: t.text, fontSize: "0.86rem" }}>{test.name}</div>
//                                 {test.description && <div style={{ fontSize: "0.71rem", color: t.faint, marginTop: 2, maxWidth: 260, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{test.description}</div>}
//                                 {test.interpretation && <div style={{ fontSize: "0.68rem", color: t.accent, marginTop: 2, display: "flex", alignItems: "center", gap: 3 }}><RiFileTextLine size={10} /> Has interpretation</div>}
//                             </div>
...
//                             {/* Param count */}
//                             <span style={{ fontSize: "0.78rem", color: test.parameters?.length ? t.text : t.faint, fontWeight: test.parameters?.length ? 500 : 400 }}>
//                                 {test.parameters?.length || 0}
//                             </span>

//                             {/* Price editor */}
//                             <PriceEditor test={test} t={t} onPriceSaved={handlePriceSaved} onToast={showToast} />

//                             {/* Toggle */}
//                             <Toggle test={test} t={t} onToggled={handleToggled} onToast={showToast} />

//                             {/* Edit button */}
//                             <button onClick={() => setModal(test)} title="Edit test"
//                                 style={{ background: t.accentBg, border: `1px solid ${t.accentRing}`, borderRadius: 7, width: 30, height: 30, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: t.accent }}>
...
//                     ✏️ Set price · Toggle to enable · Click edit icon to update test details and parameters
//                 </p>
//             )}
//         </div>
//     );
 ...
 *   heading → bold section label, no value entry, used to group sub-fields
 *
 * isSubField: true  → indented under the heading above it
 * isSubField: false → top-level row
 */
...
// All field types including heading
const FIELD_TYPES = [
...
const emptyParam = (isHeading = false) => ({
    _uid: uid(),
    name: "",
    fieldType: isHeading ? "heading" : "numeric",
    unit: "",
    rangeMin: "",
    rangeMax: "",
    rangeText: "",
    options: [],          // for fieldType "option"
    isSubField: false,
    showInReport: true,
});
...
function ParamRow({ param, idx, t, total, onChange, onDelete, onMove }) {
    ...
    const inp = (key, extraStyle = {}) => (
        <input
            ...
            onChange={e => onChange(idx, key, e.target.value)}
            style={{
                background: t.inputBg, border: `1px solid ${t.accentRing}`,
                borderRadius: 7, padding: "6px 9px", color: t.text,
                fontSize: "0.79rem", fontFamily: "'DM Sans',sans-serif",
                outline: "none", width: "100%", ...extraStyle,
            }}
        />
    );

    // ── HEADING row — simpler layout ──────────────────────────────
    if (param.fieldType === "heading") {
        return (
            <div style={{
                ...
                <span style={{ fontSize: "0.68rem", fontWeight: 700, color: t.accent, letterSpacing: "0.06em", textTransform: "uppercase", minWidth: 56 }}>
                    HEADING
                </span>
                <input
                    value={param.name}
    ...
    return (
        <div style={{
            display: "grid",
            gridTemplateColumns: "14px 1.8fr 0.85fr 0.65fr 0.55fr 0.55fr 1fr 80px",
            ...
            >
                {FIELD_TYPES.filter(f => f.value !== "heading").map(f =>
                    <option key={f.value} value={f.value}>{f.label}</option>
                )}
            </select>

            {/* Unit (not shown for option) */}
            ...
            {/* Min (numeric only) */}
            {param.fieldType === "numeric"
                ? inp("rangeMin", { placeholder: "Min", type: "number" })
                : <span />
            }

            {/* Max (numeric only) */}
            {param.fieldType === "numeric"
                ? inp("rangeMax", { placeholder: "Max", type: "number" })
                : <span />
            }
            ...
            <div style={{ display: "flex", gap: 4, justifyContent: "flex-end" }}>
                ...
                <button
                    type="button"
                    onClick={() => onChange(idx, "isSubField", !param.isSubField)}
                    title={param.isSubField ? "Make top-level" : "Make sub-field (indent)"}
                    style={{
                        width: 26, height: 26, borderRadius: 6, cursor: "pointer",
                        border: `1px solid ${param.isSubField ? t.accent : t.border}`,
                        background: param.isSubField ? t.accentBg : "none",
                        color: param.isSubField ? t.accent : t.muted,
...
function MoveBtn({ idx, total, onMove, t }) {
    return (
...
function TestFormModal({ t, isDark, editTest, onClose, onSuccess }) {
    ...
    // ── Param mutations ──────────────────────────────────────────
    const addParam = (isHeading = false) => setParams(p => [...p, emptyParam(isHeading)]);
    const delParam = (i) => setParams(p => p.filter((_, j) => j !== i));
    const changeParam = (i, k, v) => setParams(p => p.map((r, j) => j === i ? { ...r, [k]: v } : r));
    const moveParam = (i, dir) => setParams(p => {
        const arr = [...p];
        const to = i + dir;
    ...
    const handleSubmit = async () => {
        ...
        const payload = {
            ...info,
            parameters: params.map(({ _uid, ...rest }, i) => ({ ...rest, order: i })),
            interpretation,
        };
        try {
            setSaving(true);
    ...
    const TabBtn = ({ k, label, Icon }) => (
        <button type="button" onClick={() => setActiveTab(k)} style={{
            display: "flex", alignItems: "center", gap: 6,
            padding: "9px 16px", border: "none", borderRadius: "0px",
            background: "none",
            color: activeTab === k ? t.accent : t.navText,
    ...
    // ── Parameter count by type (for the tab label) ───────────────
            ...
            <div style={{
                ...
                <div style={{ padding: "20px 24px 0", borderBottom: `1px solid ${t.border}`, flexShrink: 0 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                        <div>
                            <div className="playfair" style={{ fontSize: "1.12rem", fontWeight: 700, color: t.heading }}>
                                {isEdit ? `Edit: ${editTest.name}` : "Add New Test"}
                            </div>
                            <p style={{ fontSize: "0.76rem", color: t.muted, marginTop: 3 }}>
                                {isEdit ? "Update test info, parameters and interpretation." : "Adds to global library — all vendors can see and set their own price."}
                            </p>
                        </div>
                        <button onClick={onClose} style={{ background: t.accentBg, border: `1px solid ${t.border}`, borderRadius: 8, width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: t.muted }}>
                            <RiCloseLine size={16} />
                ...
                <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
                    ...
                    {activeTab === "parameters" && (
                        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                            ...
                            {/* Column labels for normal rows */}
                            <div style={{
                                display: "grid",
                                gridTemplateColumns: "14px 1.8fr 0.85fr 0.65fr 0.55fr 0.55fr 1fr 80px",
                                gap: 6, padding: "0 12px",
                                fontSize: "0.63rem", fontWeight: 600, color: t.muted,
                                letterSpacing: "0.07em", textTransform: "uppercase", marginBottom: 4,
                            }}>
                                <span /><span>Parameter Name</span><span>Type</span><span>Unit</span>
                                <span>Min</span><span>Max</span><span>Range / Options</span><span />
                            </div>

                            {/* Parameter rows */}
                            {params.map((p, i) => (
                                <ParamRow
                                    key={p._uid} param={p} idx={i} total={params.length} t={t}
                                    onChange={changeParam} onDelete={delParam} onMove={moveParam}
                                />
                            ))}
                            ...
                            <div style={{ background: t.accentBg, border: `1px solid ${t.border}`, borderRadius: 10, padding: "12px 16px", marginTop: 8 }}>
                                ...
                                <div style={{ display: "flex", flexWrap: "wrap", gap: 16, fontSize: "0.78rem", color: t.muted }}>
                                    <span><strong style={{ color: t.text }}>Heading</strong> — bold section label in report, no value entry (e.g. "RBC Indices")</span>
                            ...
                            {params.length > 0 && (
                                <div style={{ marginTop: 12 }}>
                                    <div style={{ fontSize: "0.68rem", fontWeight: 600, color: t.muted, letterSpacing: "0.07em", textTransform: "uppercase", marginBottom: 8 }}>Report Preview</div>
                                    <div style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: 10, overflow: "hidden" }}>
                                        {/* Header row */}
                                        <div style={{ display: "grid", gridTemplateColumns: "3fr 1fr 1fr 1fr", padding: "8px 16px", background: t.accentBg, fontSize: "0.67rem", fontWeight: 600, color: t.muted, letterSpacing: "0.07em", textTransform: "uppercase", borderBottom: `1px solid ${t.border}` }}>
                                            <span>Test / Parameter</span><span>Result</span><span>Reference</span><span>Unit</span>
                                        </div>
                                        {params.map((p, i) => {
                                            if (p.fieldType === "heading") return (
                                                <div key={p._uid} style={{ padding: "8px 16px", borderBottom: `1px solid ${t.border}`, fontSize: "0.82rem", fontWeight: 700, color: t.heading, background: "rgba(56,189,248,0.03)" }}>
                                                    {p.name || <span style={{ color: t.faint, fontStyle: "italic" }}>Section heading</span>}
                                                </div>
                                            );
                                            const range = p.rangeText || (p.rangeMin !== "" && p.rangeMax !== "" ? `${p.rangeMin} - ${p.rangeMax}` : "—");
                                            return (
                                                <div key={p._uid} style={{ display: "grid", gridTemplateColumns: "3fr 1fr 1fr 1fr", padding: "7px 16px", borderBottom: i < params.length - 1 ? `1px solid ${t.border}` : "none", fontSize: "0.8rem", paddingLeft: p.isSubField ? 32 : 16 }}>
                                                    <span style={{ color: t.text }}>{p.name || <span style={{ color: t.faint, fontStyle: "italic" }}>Parameter name</span>}</span>
                                                    <span style={{ color: t.faint, fontStyle: "italic" }}>—</span>
                                                    <span style={{ color: t.muted }}>{range}</span>
                                                    <span style={{ color: t.muted }}>{p.unit || "—"}</span>
                                                </div>
                                            );
                                        })}
                                    </div>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                            ...
                            {interpretation.trim() && (
                                <div>
                                    ...
                                    <div style={{ background: t.accentBg, border: `1px solid ${t.accentRing}`, borderRadius: 10, padding: "16px", fontSize: "0.84rem", color: t.text, lineHeight: 1.75, whiteSpace: "pre-wrap" }}>
                                        {interpretation}
                                    </div>
                                </div>
                            )}
                ...
                <div style={{ padding: "14px 24px", borderTop: `1px solid ${t.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
                    ...
                    <div style={{ display: "flex", gap: 10 }}>
                        ...
                        {activeTab !== "interpretation" ? (
                            <button type="button"
                                onClick={() => setActiveTab(activeTab === "info" ? "parameters" : "interpretation")}
                                style={{ padding: "9px 20px", borderRadius: 9, border: "none", background: t.accentBg, color: t.accent, fontSize: "0.86rem", fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", display: "flex", alignItems: "center", gap: 6 }}>
                                Next <RiArrowRightLine size={14} />
                            </button>
                        ) : (
...
// ── Inline Price Editor ────────────────────────────────────────────
function PriceEditor({ test, t, onSaved, onToast }) {
    const [editing, setEditing] = useState(false);
    const [value, setValue] = useState(test.vendorPrice ?? "");
    const [saving, setSaving] = useState(false);
    const ref = useRef();
    ...
    return (
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <span style={{ fontSize: "0.87rem", fontWeight: 600, color: test.priceSet ? t.heading : t.faint }}>
                {test.priceSet ? `₹${test.vendorPrice}` : "—"}
            </span>
            <button onClick={() => { setValue(test.vendorPrice ?? ""); setEditing(true); }}
                title={test.priceSet ? "Edit price" : "Set price"}
                style={{ background: t.accentBg, border: `1px solid ${t.accentRing}`, borderRadius: 6, width: 26, height: 26, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: t.accent }}>
                <RiPencilLine size={12} />
            </button>
        </div>
...
function Toggle({ test, t, onToggled, onToast }) {
    ...
    return (
        <button onClick={handle} disabled={loading}
            title={!test.priceSet ? "Set price first" : (isOn ? "Disable" : "Enable")}
            style={{ width: 42, height: 24, borderRadius: 12, background: isOn ? t.accent : "rgba(148,163,184,0.25)", border: "none", cursor: loading ? "not-allowed" : "pointer", position: "relative", transition: "background .2s", opacity: loading ? 0.6 : 1 }}>
            <span style={{ position: "absolute", top: 3, left: isOn ? 20 : 3, width: 18, height: 18, borderRadius: "50%", background: "#fff", transition: "left .2s", boxShadow: "0 1px 4px rgba(0,0,0,0.2)" }} />
        </button>
    );
...
export default function VendorTest({ t, isDark }) {
    ...
    const stats = useMemo(() => ({
        ...
        priced: tests.filter(t => t.priceSet).length,
        active: tests.filter(t => t.vendorIsActive && t.priceSet).length,
        params: tests.reduce((s, t) => s + (t.parameters?.filter(p => p.fieldType !== "heading").length || 0), 0),
    }), [tests]);

    const handleSaved = useCallback((saved, isEdit) => {
        if (isEdit) setTests(ts => ts.map(t => t._id === saved._id ? { ...saved, vendorPrice: t.vendorPrice, priceSet: t.priceSet, vendorIsActive: t.vendorIsActive } : t));
        else setTests(ts => [{ ...saved, vendorPrice: null, priceSet: false, vendorIsActive: false }, ...ts]);
        showToast(isEdit ? "Test updated." : "Test added to library!", "success");
    }, [showToast]);

    const handlePriceSaved = useCallback((id, price) => setTests(ts => ts.map(t => t._id === id ? { ...t, vendorPrice: price, priceSet: true } : t)), []);
    const handleToggled = useCallback((id) => setTests(ts => ts.map(t => t._id === id ? { ...t, vendorIsActive: !t.vendorIsActive } : t)), []);
      ...
      `}</style>
            ...
            {loading ? (
                ...
                <div style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: 16, overflow: "hidden" }}>
                    ...
                    {/* Rows */}
                    {visible.map((test, i) => (
                        <div key={test._id} className="cat-row" style={{ display: "grid", gridTemplateColumns: "2.2fr 1fr 1.1fr 0.7fr 0.6fr 0.6fr 0.9fr 0.6fr 0.6fr", padding: "12px 20px", borderBottom: i < visible.length - 1 ? `1px solid ${t.border}` : "none", alignItems: "center", transition: "background .12s" }}>
                            ...
                            {/* Fields count (excluding headings) */}
                            <span style={{ fontSize: "0.82rem", fontWeight: 500, color: test.parameters?.filter(p => p.fieldType !== "heading").length ? t.text : t.faint }}>
                                {test.parameters?.filter(p => p.fieldType !== "heading").length || 0}
                            </span>

                            <PriceEditor test={test} t={t} onSaved={handlePriceSaved} onToast={showToast} />
                            <Toggle test={test} t={t} onToggled={handleToggled} onToast={showToast} />
