...
/**
 * VendorAnalysis.jsx  —  OPTIMIZED
 *
 * What changed vs the original, and why it matters at scale (thousands of bills):
 *
 * 1. STATS + REFERRAL + TOP-TESTS are now computed in a SINGLE pass over
 *    `rangeBills` (one `for` loop building three maps), instead of ~6 separate
 *    .filter()/.reduce() scans. O(6n) -> O(n).
 *
 * 2. INCOME / PATIENT CHARTS no longer re-filter the full `bills` array once
 *    per bucket (was O(bills * days) — the single biggest cost with bulk data).
 *    Instead we build a `dayBuckets` Map ONCE per `bills` change (O(n)), then
 *    every daily/weekly/monthly grouping just walks the small, sorted list of
 *    DISTINCT billing days and sums pre-aggregated bucket totals. This is
 *    O(distinct_days) per period switch, which stays small even at 100k bills.
 *
 * 3. "New vs returning patient" no longer sorts the whole bills array
 *    (O(n log n) on every `bills` change). A single O(n) pass tracks the min
 *    billing date per patient using plain string comparison (safe because
 *    dates are formatted YYYY-MM-DD, which sorts lexically = chronologically).
 *
 * 4. Animations (count-up, donut, bars) moved from `setInterval(16ms)` to
 *    `requestAnimationFrame`, so they're synced to the browser's paint cycle,
 *    pause correctly in background tabs, and don't fight each other for main
 *    thread time when 5 stat cards + 2 donuts + 2 bar charts animate together.
 *
 * 5. Sub-components (StatCard, DonutChart, BarChart, Section, ReferralTable,
 *    TopTestsTable) are wrapped in React.memo, so typing in the referral
 *    search box — or changing a date filter — no longer re-renders the whole
 *    dashboard, only the piece that actually changed.
 *
 * 6. Referral search input is decoupled from the filtered list via
 *    `useDeferredValue`, so keystrokes stay snappy even with a large referral
 *    list, and the (potentially long) referral table is paginated client-side
 *    instead of rendering every row at once.
 *
 * NOTE ON TRUE SCALE: this file still fetches up to 2000 raw bills and
 * aggregates them in the browser. That's fine up to a few thousand rows.
 * If you expect vendors to accumulate tens of thousands of bills, the next
 * step is to move stats/income/patient aggregation to the backend (a
 * `/api/analytics?from=&to=&groupBy=` endpoint that returns pre-summed
 * buckets) so the client never downloads raw rows at all — see note at the
 * bottom of this file.
 *
 * Props: t (theme), isDark (boolean)
 * APIs:  /api/bills  /api/reports  /api/patients
...
function useRafProgress(active, duration = 700, resetKey) {
    ...
    useEffect(() => {
        ...
        // resetKey intentionally re-triggers the animation when the data changes
...
/**
 * NEXT STEP FOR REAL SCALE (thousands of USERS across many vendors, not just
 * thousands of bills for one vendor):
 *
 * Everything above still ships raw bill rows to the browser and aggregates
 * them client-side. That's the right trade-off up to a few thousand bills
 * per vendor per view. Beyond that, move aggregation server-side:
 *
 *   GET /api/analytics/summary?vendorId=&from=&to=
 *     -> { registrations, tests, billing, paid, due, referralBreakdown, topTests }
 *
 *   GET /api/analytics/timeseries?vendorId=&groupBy=daily|weekly|monthly&from=&to=
 *     -> [{ label, paid, due, uniquePatients, newPatients }, ...]
 *
 * Computed with MongoDB aggregation pipelines ($match on date range, $group
 * by day, $facet for the multiple breakdowns in one round trip). The frontend
 * then renders whatever comes back — no client-side reduce over raw bills at
 * all, and payload size stops scaling with bill count.
 */