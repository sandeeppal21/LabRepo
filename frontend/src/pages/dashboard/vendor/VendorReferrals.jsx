...
// Uses referralService.js — zero dummy data
//
// Performance pass (matches BillRegistration/ReportEntry/VendorTest pattern):
//   1. Row extracted into a React.memo'd component so editing/typing in the
//      modal or toggling one row's hover state doesn't re-render every row.
//   2. In-flight fetch is cancelled via AbortController whenever a newer
//      request starts (tab/page/search change) or the component unmounts,
//      so a slow response can never overwrite fresher state ("stale
//      response" race).
//   3. Row hover uses a CSS class + custom property instead of
//      onMouseEnter/onMouseLeave JS handlers (removes 2 handler allocations
//      + a style write per row per hover).
//   4. Virtualization intentionally NOT added here: this table is
//      server-paginated at pagination.pageSize (10 rows/page), so at most
//      10 DOM rows ever mount at once. react-window/@tanstack/react-virtual
//      only pays off once a single render can contain hundreds of rows
//      (e.g. an unpaginated or "load all" list) — adding it here would be
//      pure overhead. If this table ever gets a "show all" / large
//      page-size mode, virtualize at that point using the same
//      row-height-based FixedSizeList approach used elsewhere.
...
const ROW_GRID = "52px 2fr 1.3fr 1.4fr 1fr 1.6fr 1.1fr 100px";
...
// ════════════════════════════════════════════════════════════
// Memoized row — only re-renders when its own entry, index, theme,
// or the two stable callbacks change. Parent re-renders (search typing,
// modal open/close, hover on a *different* row) no longer touch this.
// ════════════════════════════════════════════════════════════
const ReferralRow = memo(function ReferralRow({ entry, srNo, t, isLast, onEdit, onDelete }) {
    return (
        <div
            className="vref-row"
            style={{
                ...
                fontSize: "0.84rem",
                alignItems: "center",
                // custom property read by the .vref-row:hover rule below —
                // swaps the hover color per-theme without a JS handler
                "--vref-hover": t.rowHover,
            }}
        >
            {/* Sr */}
            <span style={{ fontSize: "0.74rem", color: t.faint, fontWeight: 600 }}>
            ...
            {/* Actions */}
            <div style={{ display: "flex", gap: 6 }}>
                <button onClick={() => onEdit(entry)} title="Edit" className="vref-icon-btn"
                    style={{ background: t.accentBg, border: `1px solid ${t.accentRing}`, color: t.accent, borderRadius: 8, padding: "5px 9px", cursor: "pointer", display: "flex", alignItems: "center" }}>
                    <RiEditLine size={13} />
                </button>
                <button onClick={() => onDelete({ id: entry._id, name: entry.name })} title="Delete" className="vref-icon-btn"
                    style={{ background: "rgba(239,68,68,.08)", border: "1px solid rgba(239,68,68,.2)", color: "#ef4444", borderRadius: 8, padding: "5px 9px", cursor: "pointer", display: "flex", alignItems: "center" }}>
                    <RiDeleteBinLine size={13} />
                </button>
            </div>
...
export default function VendorReferrals({ t, isDark }) {
    ...
    // ── Fetch, with request cancellation ────────────────────
    // Every call aborts whatever request is still in flight before starting
    // a new one, and the effect's cleanup aborts on unmount/dep-change too.
    // This guarantees a slow/late response for a stale (tab, page, search)
    // combo can never land and overwrite the entries the user is now
    // looking at.
    ...
    const load = useCallback(async () => {
        ...
        try {
            const res = await fetchReferrals({
                ...
                page,
                search,
                signal: controller.signal, // referralService must forward this to axios' config
            });
            setEntries(res.data.data);
            setPagination(res.data.pagination);
        } catch (err) {
            if (err.name === "CanceledError" || err.name === "AbortError" || err.code === "ERR_CANCELED") {
                return; // superseded by a newer request — not a real error
            }
            setApiError(err.response?.data?.message || err.message);
            setEntries([]);
        } finally {
    ...
    // Stable identity so ReferralRow's memoization isn't defeated by a
    // fresh function reference every parent render.
    ...
    return (
        <div style={{ position: "relative" }}>
            ...
            {/* Row/icon-button hover now lives in CSS, not JS handlers */}
            <style>{`
                .vref-row { background: transparent; transition: background .12s; }
                .vref-row:hover { background: var(--vref-hover); }
                .vref-icon-btn { transition: opacity .12s; }
                .vref-icon-btn:hover { opacity: .7; }
            ...
            <div style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: 16, overflow: "hidden" }}>
                ...
                {/* Header row */}
                <div style={{ display: "grid", gridTemplateColumns: ROW_GRID, padding: "10px 20px", background: t.accentBg, borderBottom: `1px solid ${t.border}`, fontSize: "0.67rem", fontWeight: 700, color: t.muted, letterSpacing: "0.08em", textTransform: "uppercase" }}>
                    <span>Sr.</span>
                    <span>Name</span>
                    <span>Phone</span>
                    <span>Degree</span>
                ...
                {/* Data rows — each a memoized ReferralRow */}
                {!loading && !apiError && entries.map((entry, i) => (
                    <ReferralRow
                        key={entry._id}
                        entry={entry}
                        srNo={(pagination.page - 1) * pagination.pageSize + i + 1}
                        t={t}
