 ...
 *
 * Billing page with:
 *  - Summary stat cards (Total, Paid, Due, Bills count) — from server-computed totals
 *  - Bills table — Bill Date, Patient, Tests, Ref Doctor, Amount, Paid, Due, Net
 *  - Server-side search, date filter, status filter, pagination ("Load more")
 *  - Virtualized rows (@tanstack/react-virtual) — only visible rows are in the DOM
 *  - Expandable row — shows test breakdown on + click
 *
 * Props: t (theme — MUST be a stable/memoized reference from the parent, or
 *            React.memo below won't skip re-renders), isDark, onPrintBill
 * File:  src/pages/dashboard/vendor/VendorBilling.jsx
 * API:   src/services/billingService.js → getBills()
 * Deps:  npm install @tanstack/react-virtual
 *
 * Perf notes (2026-07 refactor):
 *  - Search/status filtering + all summary totals happen SERVER-SIDE. The
 *    client sends { from, to, search, status, page, limit }; the server
 *    returns { bills, hasMore, totals, paymentCounts } computed across the
 *    FULL filtered set, not just the loaded page. (Backend change pending
 *    — see chat.)
 *  - Expanded-row state is a Set, not a plain object.
 *  - Rows are virtualized with dynamic height (expand changes height) via
 *    react-virtual's measureElement/ResizeObserver path.
 *  - Row hover is a CSS class (.vb-row:hover) instead of onMouseEnter/Leave.
 *  - Sub-components (PatientInfo, TestRow, ExpandedBillDetails, BillRow,
 *    PaymentStatusPill) are React.memo'd so expanding one row or typing in
 *    search doesn't re-render the whole table — kept in this single file
 *    per request, not split into separate files.
 *  - Bill dates are formatted through a small module-level cache so 100+
 *    rows don't each re-run Date parsing/formatting every render.
 */
...
// ── Date formatting cache — avoids re-parsing/re-formatting the same
//    billingDate on every render for every row ─────────────────────
...
// ── BillRow — memoized, forwardRef so react-virtual can measure its
//    real DOM height (needed since expand/collapse changes row height) ──
